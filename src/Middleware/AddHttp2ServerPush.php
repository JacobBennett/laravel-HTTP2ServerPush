...
class AddHttp2ServerPush
{
    ...
    protected function fetchLinkableNodes($response)
    {
        ...
        return collect($crawler->filter('link:not([rel*="icon"]):not([rel="preconnect"]):not([rel="canonical"]), script[src], img[src]:not([loading="lazy"]), object[data]')->extract(['src', 'href', 'data']));
    }
